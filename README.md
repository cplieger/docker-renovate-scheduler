...
- Runs `renovate` on a **built-in interval** (`SCHED_INTERVAL=6h`): one run at startup for immediate freshness, then every interval. Or set `SCHED_INTERVAL=off` and trigger each run **externally** (`docker exec … run`).
