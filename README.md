...
Everything else is Renovate's own configuration, with one exception: the scheduler sets `RENOVATE_NODE_ARGS` on every run and overrides any value you pass (see [Healthcheck](#healthcheck)). The essentials for a self-hosted bot:
...
A Renovate process that fails before it reaches its first repository (a broken install, a missing module in the base image) counts as a failed run: the scheduler logs `renovate run failed`, the `run` command exits 1, and health flips. Renovate on its own only logs such a bootstrap failure and exits 0, so the scheduler starts Renovate's node with `--unhandled-rejections=strict` through `RENOVATE_NODE_ARGS`, a variable read by the base image's `renovate` launcher script rather than a documented Renovate option. The scheduler owns that variable and overrides any value you set; put your own node flags in `NODE_OPTIONS`, which reaches Renovate unchanged. The flag also applies mid-run: an unhandled rejection there ends the process at once with node's stack trace on stderr, instead of Renovate logging it and finishing the remaining repositories before exiting 1. The run is reported as failed either way.
